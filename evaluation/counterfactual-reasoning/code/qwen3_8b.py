...
def parse_args():
    ...
    parser.add_argument(
        "--output_csv",
        type=str,
        default="results/qwen/output_qwen_metrics.csv",
        help="Path to save per-example results"
    )
    parser.add_argument(
        "--summary_csv",
        type=str,
        default="results/qwen/output_qwen_summary.csv",
        help="Path to save summary statistics"
    )
    parser.add_argument(
        "--pairwise_csv",
        type=str,
        default="results/qwen/output_qwen_pairwise.csv",
...
def score_suffix_conditional_nll(
    ...
    """
    原始指标的 causal LM 版本：
    只计算 suffix 在 prefix 条件下的负对数似然。
    这最接近你原来“mask最后一个词再看loss”的意图。
    """
    if prefix is None:
        prefix = ""
    if suffix is None:
